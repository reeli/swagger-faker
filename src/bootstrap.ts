...
import {putBackRefs} from "./putBackAllRefs";
...
const handleResponse = (spec: IOpenAPI, resp?: IReference | IResponse) => {
    if (getRef(resp)) {
        getPathsFromRef(resp.$ref);
        return;
    }

    return putBackRefs(store, spec.components?.schemas ?? {}, getFirstValue(resp?.content).schema);
};
